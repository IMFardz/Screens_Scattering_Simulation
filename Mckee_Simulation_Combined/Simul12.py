...
veff2 = np.dot(pulsar.vel.get_xyz(), s2.normal.get_xyz())*(1-s_2p)/s_2p
...
delay_jb2 = -((jodrell_b / veff2) * np.dot(jodrell_u, veff_u)).to(u.s)
delay_vla2 = -((vla_b / veff2) * np.dot(vla_u, veff_u)).to(u.s)
...
eta1 = ((lambd**2/(2*ac.c)) * deff/(veff**2)).to(u.s**3)
...
vla_b = np.linalg.norm( vla.pos.get_xyz())
vla_u = vla.pos.get_xyz() / vla_b
delay_jb1 = -((jodrell_b / veff) * np.dot(jodrell_u, veff_u)).to(u.s)
delay_vla1 = -((vla_b / veff) * np.dot(vla_u, veff_u)).to(u.s)
...
obs_ar2 = arecibo.observe(
    s1.observe(s2.observe(pulsar, distance=dp-d2), distance=d2-d1), distance=d1)
path_shape = obs_ar2.tau.shape  # Also trigger calculation of pos, vel.
tpos = obs_ar2.pos
scat1 = obs_ar2.source.pos
scat2 = obs_ar2.source.source.pos
ppos = obs_ar2.source.source.source.pos
x = np.vstack(
    [np.broadcast_to(getattr(pos, 'x').to_value(u.AU), path_shape).ravel()
...
tau0 = np.hstack([obs_ar1.tau.ravel(), obs_ar2.tau.ravel()])
taudot = np.hstack([obs_ar1.taudot.ravel(), obs_ar2.taudot.ravel()])
brightness = np.hstack([np.broadcast_to(obs_ar1.brightness, obs_ar1.tau.shape).ravel(),
                        np.broadcast_to(obs_ar2.brightness, obs_ar2.tau.shape).ravel()])
...
obs_jb2 = jodrell.observe(s1.observe(s2.observe(pulsar, distance=dp-d2), distance=d2-d1), distance=d1)
path_shape = obs_jb2.tau.shape  # Also trigger calculation of pos, vel.
tpos = obs_jb2.pos
scat1 = obs_jb2.source.pos
scat2 = obs_jb2.source.source.pos
ppos = obs_jb2.source.source.source.pos
x = np.vstack(
    [np.broadcast_to(getattr(pos, 'x').to_value(u.AU), path_shape).ravel()
     for pos in (tpos, scat1, scat2, ppos)])
y = np.vstack(
...
# Create dynamic spectrum using delay for each path.
tau0 = np.hstack([obs_jb1.tau.ravel(), obs_jb2.tau.ravel()])
taudot = np.hstack([obs_jb1.taudot.ravel(), obs_jb2.taudot.ravel()])
brightness = np.hstack([np.broadcast_to(obs_jb1.brightness, obs_jb1.tau.shape).ravel(),
                        np.broadcast_to(obs_jb2.brightness, obs_jb2.tau.shape).ravel()])
t = np.linspace(0, 120*u.min, 300)[:, np.newaxis]
f = np.linspace(300*u.MHz, 310*u.MHz, 600)
...
obs_vla2 = vla.observe(s1.observe(s2.observe(pulsar, distance=dp-d2), distance=d2-d1), distance=d1)
path_shape = obs_vla2.tau.shape  # Also trigger calculation of pos, vel.
tpos = obs_vla2.pos
scat1 = obs_vla2.source.pos
scat2 = obs_vla2.source.source.pos
ppos = obs_vla2.source.source.source.pos
x = np.vstack(
    [np.broadcast_to(getattr(pos, 'x').to_value(u.AU), path_shape).ravel()
     for pos in (tpos, scat1, scat2, ppos)])
y = np.vstack(
...
# Create dynamic spectrum using delay for each path.
tau0 = np.hstack([obs_vla1.tau.ravel(), obs_vla2.tau.ravel()])
taudot = np.hstack([obs_vla1.taudot.ravel(), obs_vla2.taudot.ravel()])
brightness = np.hstack([np.broadcast_to(obs_vla1.brightness, obs_vla1.tau.shape).ravel(),
                        np.broadcast_to(obs_vla2.brightness, obs_vla2.tau.shape).ravel()])
t = np.linspace(0, 120*u.min, 300)[:, np.newaxis]
f = np.linspace(300*u.MHz, 310*u.MHz, 600)
tau = (tau0[:, np.newaxis, np.newaxis] + taudot[:, np.newaxis, np.newaxis] * t)
ph = phasor(f, tau)
...
plt.plot(fd, (fd**2 * eta1).to(u.us), color='red')
plt.plot(fd, (fd**2 * eta2).to(u.us), color='orange')
...
plt.xlim(-5, 5)
plt.ylim(0, 15)
#plt.title("$\\eta = {0:.5f}$".format(eta2))
plt.colorbar()

fig.add_subplot(223)
plt.plot(fd, ccw_average_arjb, label="Delay Averaged  AR-JB")
plt.plot(fd, fd * delay_jb1 * (2*np.pi) / 1e3, label = "Delay = {0:.3f}".format(delay_jb1), color='red', linestyle='dotted')
plt.plot(fd, fd * delay_jb2 * (2*np.pi) / 1e3, label = "Delay = {0:.3f}".format(delay_jb2), color='orange', linestyle='dotted')
plt.xlabel("Doppler Frequency [mHz]")
plt.ylabel("Radians")
plt.legend()
plt.xlim(-5, 5)
...
plt.plot(fd, fd * delay_vla1 * (2*np.pi) / 1e3, label = "Delay = {0:.3f}".format(delay_vla1), color='red', linestyle='dotted')
plt.plot(fd, fd * delay_vla2 * (2*np.pi) / 1e3, label = "Delay = {0:.3f}".format(delay_vla2), color='orange', linestyle='dotted')
plt.xlabel("Doppler Frequency [mHz]")
plt.ylabel("Radians")
plt.legend()
plt.xlim(-5, 5)
