...
tau0 = np.hstack([obs_ar2.tau.ravel()])
taudot = np.hstack([obs_ar2.taudot.ravel()])
brightness = np.hstack([np.broadcast_to(obs_ar2.brightness, obs_ar2.tau.shape).ravel()])
...
# Create dynamic spectrum using delay for each path.
tau0 = np.hstack([obs_jb2.tau.ravel()])
taudot = np.hstack([obs_jb2.taudot.ravel()])
brightness = np.hstack([np.broadcast_to(obs_jb2.brightness, obs_jb2.tau.shape).ravel()])
t = np.linspace(0, 120*u.min, 300)[:, np.newaxis]
f = np.linspace(300*u.MHz, 310*u.MHz, 600)
...
# Create dynamic spectrum using delay for each path.
tau0 = np.hstack([obs_vla2.tau.ravel()])
taudot = np.hstack([obs_vla2.taudot.ravel()])
brightness = np.hstack([np.broadcast_to(obs_vla2.brightness, obs_vla2.tau.shape).ravel()])
t = np.linspace(0, 120*u.min, 300)[:, np.newaxis]
f = np.linspace(300*u.MHz, 310*u.MHz, 600)
tau = (tau0[:, np.newaxis, np.newaxis] + taudot[:, np.newaxis, np.newaxis] * t)
ph = phasor(f, tau)
