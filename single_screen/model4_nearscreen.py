...
screen1 = Screen1D(CylindricalRepresentation(1., 0*u.deg, 0.).to_cartesian(),
            np.array([0.00001, -0.4])*u.AU,
...
if __name__ == '__main__':
    ...
    # ARECIBO
    obs1 = arecibo.observe(screen1.observe(pulsar, distance=dp-d1),  distance=d1)
    ...
    brightness = np.hstack([np.broadcast_to(obs1.brightness, obs1.tau.shape).ravel()])
    ...
    plot_screen(ax, screen1, d1, color='red')
    plot_screen(ax, pulsar, dp, color='green')
    obs3 = jodrell.observe(screen1.observe(pulsar, distance=dp-d1), distance=d1)
