"""
Description of model: Two Parallel Screens, 2 images (not symmetric) on first,
one image on second, only Earth observatories are moving
"""
...
s1 = Screen1D(CylindricalRepresentation(1., 45*u.deg, 0.).to_cartesian(),np.array([0.001, 0.201])*u.AU,
              magnification=np.array([1, 1]))

s2 = Screen1D(CylindricalRepresentation(1., 45*u.deg, 0.).to_cartesian(),
              np.array([0.0001])*u.AU, magnification=np.array([1]))
